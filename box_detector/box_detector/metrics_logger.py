...
class MetricsLogger(Node):
    def __init__(self):
        ...
        self.declare_parameter('corrida', 0)       # 0 = usar timestamp automatico
        self.declare_parameter('directorio_csv', '.')
        ...
        corrida_num  = self.get_parameter('corrida').value
        directorio   = self.get_parameter('directorio_csv').value

        if corrida_num == 0:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            nombre = f'metricas_lidar_{timestamp}.csv'
        else:
            nombre = f'metricas_lidar_corrida_{corrida_num:02d}.csv'

        self.archivo = f'{directorio}/{nombre}' if directorio != '.' else nombre
